...
add_minor_tickmarks(y_tick_by = 1)
```
In the 1970s the median is in the critical zone (below 0.4) but the intervals
extend into the healthy zone (above 0.8), but somewhat less so when using HDIs
compared to the ETIs. From 2014 to 2019 the ETIs are almost fully in the healthy
zone, but the HDI extend somehat into it, giving a less healthy perception of
stock recovery.
...
pdf(file = "figure-cod-relative-biomss.pdf",
    height = pdf_height,
    width = pdf_width,
    paper="special")
