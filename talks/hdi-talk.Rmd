...
output:
  beamer_presentation:
    ...
    highlight: espresso
    # pandoc_args: ["--syntax-highlighting", "default"]
    # toc_depth: 1
...
rmarkdown::render("hdi-talk.Rmd")xx
...
library(tibble)
library(pacea)
# library(kableExtra)   # Don't actually create any tables here, and think it
# messed up my simple Markdown manual ones.
...
source("../R-extra/plot-mcmc-subsample.R") 
```

```{r coverpage, echo=FALSE, out.width = "100%"}
knitr::include_graphics(paste0(here::here(),
                               "/talks/hdi-title-slide-pbs-tesa-2025.png"))
```
...
You have to summarise 8000 values.
...
\medskip
  ...
  - The `r colorize("middle 95\\%")` of values?

\medskip

  - The `r colorize("most likely 95\\%")` of values?
...
```{r hakemap, echo=FALSE, out.width = "30%", eval = FALSE}
knitr::include_graphics(paste0(here::here(),
...
\begin{columns}[onlytextwidth, T]
  ...
  \includegraphics[width=0.8\linewidth, height = 0.9\linewidth]{hake-25-survey-cropped.png}
  \end{column}
  \begin{column}{.45\linewidth}
    \begin{itemize}
	  \item midwater schooling species
	  \item live to 15-20 years
	  \item grow to about 1 kg
	  \item 50\% mature at 2-3~yrs
    \end{itemize}
  \end{column}
\end{columns}
...
- Results are from 2024 assessment (Grandin et al., 2024)
...
## MCMC samples of recruitment through time

```{r recsample1, echo=FALSE}
y_max_traceplot = 20
plot_mcmc_subsample(hake_recruitment_mcmc,
                    highlight = FALSE,
                    samples_explicit = 1,
                    samples_lwd = 2,
                    y_max = y_max_traceplot)
```

## MCMC samples of recruitment through time

```{r recsample2, echo=FALSE}
plot_mcmc_subsample(hake_recruitment_mcmc,
                    highlight = FALSE,
                    samples_explicit = 1:2,
                    samples_lwd = 2,
                    y_max = y_max_traceplot)
```
...
```{r recsample3, echo=FALSE}
plot_mcmc_subsample(hake_recruitment_mcmc,
                    highlight = FALSE,
                    samples_explicit = 1:3,
                    samples_lwd = 2,
                    y_max = y_max_traceplot)
```
...
```{r recsample4, echo=FALSE}
plot_mcmc_subsample(hake_recruitment_mcmc,
                    highlight = FALSE,
                    samples_explicit = 1:4,
                    samples_lwd = 2,
                    y_max = y_max_traceplot)
```
...
```{r recsample5, echo=FALSE}
plot_mcmc_subsample(hake_recruitment_mcmc,
                    highlight = FALSE,
                    samples_explicit = 1:5,
                    samples_lwd = 2,
                    y_max = y_max_traceplot)
```
...
```{r recsample20, echo=FALSE}
plot_mcmc_subsample(hake_recruitment_mcmc,
                    highlight = FALSE,
                    samples_explicit = 1:20,
                    samples_lwd = 2,
                    y_max = y_max_traceplot)
```

And that is only 20 of the 8000 MCMC time series for recruitment. 

Hence need to summarise the 8000 values.

## 95% credible intervals of samples of recruitment through time

```{r recsample, echo=FALSE}
plot(hake_recruitment_2024,
     xlim = c(lubridate::dmy(01012006),
              lubridate::dmy(01012025)),
     y_max = 30)
```

## Using 2021 recruitment as example

- estimate of number of age-0 fish in 2021
...
<!-- 
From .tex (then simplifying above), as generated from Markdown:

\protect\phantomsection\label{comparing-eti-and-hdi-1}
...
- relative spawning biomass is $B/B_{\mathrm{MSY}}$
- critical zone is $B/B_{\mathrm{MSY}} < 0.4$
- 95% credible intervals

\medskip

- Assessment in 2024 (Mazur, Holt, Fisch, and English, in press)
...
*PBS authors: Freshwater et al. (2019), Fisch et al. (2019, 2022), 
Fisch & Bence (2020), Ford et al. (2014)
